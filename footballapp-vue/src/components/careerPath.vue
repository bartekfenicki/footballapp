...
export default {
  data() {
    return {
      players,
      correctPlayer: { previous_clubs: [], current_club: null }, // Avoid undefined errors
      userGuess: "",
      lives: 4,
      gameOver: false,
      gameWon: false,
  ...
  methods: {
    ...
    checkGuess() {
      ...
      const guessedPlayer = this.players.find(
        (p) => p.first_name.toLowerCase() + " " + p.last_name.toLowerCase() === this.userGuess.toLowerCase()
      );
    ...
    filterSuggestions() {
      ...
      this.filteredSuggestions = this.players
        .filter(player =>
          `${player.first_name} ${player.last_name}`.toLowerCase().includes(this.userGuess.toLowerCase())
        )
        .map(player => ({ name: `${player.first_name} ${player.last_name}` }));
    },
